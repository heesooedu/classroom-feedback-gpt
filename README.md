...
- 각 문제 페이지에서
  ...
  - GPT가 채점한 **점수와 피드백** 확인
- 한 문제에 대해 여러 번 제출 가능 (최근 24시간 기준으로 제출 횟수 제한, API 비용 때문에 제한을 둠)
...
- `static/`  
  CSS, JS, 이미지 등 정적 파일
- `.env`  
  환경변수 파일 (GPT API 키, 관리자 계정 등) — **Git에 올리면 안 되며 로컬에 직접 만들어주세요**
- `requirements.txt`  
...
2. 필요하면 Git 설치 (이미 학교 PC에 설치되어 있으면 생략)
3. OpenAI 계정 및 **API 키**
...
이 명령으로 Flask, SQLAlchemy, openai, waitress 등 이 프로젝트에 필요한 모든 패키지가 설치됩니다.

---

## 6. 환경변수(.env) 설정

프로젝트 폴더(`app.py`가 있는 위치)에 **`.env` 파일**을 만듭니다.
...
내용 예시는 다음과 같습니다.
    ...
    # 관리자 계정 (기본 값이기 때문에 최초 실행전 바꿔주세요)
    ADMIN_USERNAME=admin
    ADMIN_PASSWORD=admin1234

주의사항:

- 실제 배포/수업에서는 `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `SECRET_KEY`를 어렵게 지정하기!
- 나중에 깃헙에 올려두신다고 하더라도 `.env` 파일은 **절대 GitHub에 올리면 안 됩니다!!**
...
※ `.env` 파일은 그대로 두면, 관리자 계정과 GPT 키는 그대로 재사용됩니다
